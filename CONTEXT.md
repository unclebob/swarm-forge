...
- `pending` — appended by `notify-agent.sh` into the TARGET role's logbook when that role is busy; contains the message content. Multiple entries may exist; delivered in order. `notify-agent.sh` never rejects.
- `executing` — written by the harness as step 0 of the delivery sequence (before `/clear`). Signals a task is in progress.
...
- `executed` — written by `notify-agent.sh` to the CALLING role's own logbook immediately after `sent`; signals the calling role's task is complete. The Stop hook reads this as the idle signal to deliver the next `pending` entry.
