...
create_role_session() {
  ...
  tmux -S "$TMUX_SOCKET" set-option -t "$session" set-titles on
  tmux -S "$TMUX_SOCKET" set-option -t "$session" set-titles-string "#W #T"
}
...
launch_role() {
  ...
  if [[ "$index" -eq "${CLEANUP_OWNER_INDEX}" ]]; then
    launch_cmd="${launch_cmd}; exit_code=\$?; TERM_PROGRAM='${TERM_PROGRAM:-}' nohup '$SCRIPT_DIR/swarm-cleanup.sh' '$TMUX_SOCKET' '$WINDOW_IDS_FILE'"
    local session_name
