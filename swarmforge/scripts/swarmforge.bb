...
(defn opencode-model-arg [row]
  ;; SWARMFORGE_MODEL sets a default model for opencode roles; a per-role
  ;; --model in extra-cli-args always wins.
  (when-let [model (System/getenv "SWARMFORGE_MODEL")]
    (when-not (str/includes? (or (:extra-args row) "") "--model")
      (str "--model " (sq model) " "))))
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  "opencode" (str "opencode " (sq (str role-worktree)) " " (opencode-model-arg row) (extra-args-prefix row) "--prompt \"$(cat " (sq (str prompt-file)) ")\"")))
      (= index 0)
